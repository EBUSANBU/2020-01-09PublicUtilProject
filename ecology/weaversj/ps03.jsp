...
String loginids = session.getAttribute("loginids").toString();
if(loginids.equals("sysadmin")){
	rs.executeSql("select * from hrmresourcemanager where loginid='"+loginids+"' ");
}else{
	rs.executeSql("select * from HrmResource where loginid='"+loginids+"'  and status<4");
}

if(rs.next()){//OA中有相关的人员
	...
	request.getSession(true).setMaxInactiveInterval(86400);
	...
	request.getRequestDispatcher("/wui/main.jsp").forward(request, response);
	
    
    if (user_new.getUID() != 1) {
    	request.getSession(true).setAttribute("accounts", new weaver.login.VerifyLogin().getAccountsById(user_new.getUID()));
    }
    request.getSession(true).setAttribute("moniter", new OnLineMonitor("" + user_new.getUID(), user_new.getLoginip()));
...
{
  String str1 = "true";
  String str2 = paramHttpServletRequest.getHeader("User-Agent").toLowerCase();
  if ((str2.indexOf("rv:11") == -1) && (str2.indexOf("msie") == -1)) {
    str1 = "false";
  }
  if ((str2.indexOf("rv:11") > -1) || (str2.indexOf("msie") > -1)) {
    str1 = "true";
