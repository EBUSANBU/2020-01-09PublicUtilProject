...
<jsp:useBean id="mrr" class="weaver.meeting.Maint.MeetingRoomReport"
	scope="page" />
<jsp:useBean id="SptmForMeeting"
	class="weaver.splitepage.transform.SptmForMeeting" scope="page" />
...
<jsp:useBean id="meetingSetInfo"
	class="weaver.meeting.Maint.MeetingSetInfo" scope="page" />
<jsp:useBean id="ManageDetachComInfo"
	class="weaver.hrm.moduledetach.ManageDetachComInfo" scope="page" />
<%!public String getTimesBg(int j, int dspUnit) {
	...
	public String getDayOccupied(String thisDate, List beginDateList, List beginTimeList, List endDateList,
			List endTimeList, List cancelList) {
	...
	public String getHourOccupied(String thisDate, String thisHour, List beginDateList, List beginTimeList,
			List endDateList, List endTimeList, List cancelList, int dspUnix) {
		String[] minute = new String[24 * 60];
		String starttime1 = dspUnix == 1
				? ((thisHour.length() == 1 ? "0" + thisHour : thisHour) + ":00")
				: getTimesBg(Util.getIntValue(thisHour) - 1, dspUnix);
		String endtime1 = dspUnix == 1
				? ((thisHour.length() == 1 ? "0" + thisHour : thisHour) + ":59")
				: getTimesEd(Util.getIntValue(thisHour), dspUnix);
		...
		for (int i = 0; i < beginDateList.size(); i++) {
			...
			if (!"1".equals(cancel)
					&& (beginDate.compareTo(thisDate) < 0
							|| (beginDate.compareTo(thisDate) == 0 && beginTime.compareTo(endtime1) <= 0))
					&& (thisDate.compareTo(endDate) < 0
							|| (thisDate.compareTo(endDate) == 0 && (starttime1).compareTo(endTime) <= 0))) {
...
<%
	...
	String nowdate = Util.add0(today.get(Calendar.YEAR), 4) + "-" + Util.add0(today.get(Calendar.MONTH) + 1, 2)
	...
	if (operation.equals("cancel")) {
		RecordSet.executeSql("update meeting set cancel='1',meetingStatus=4,canceldate='" + nowdate
				+ "',canceltime='" + nowtime + "' where id=" + meetingid);
	}
	...
	String currenttodate = Util.add0(currentyear, 4) + "-" + Util.add0(currentmonth, 2) + "-"
			+ Util.add0(currentday, 2);
	...
	switch (bywhat) {
		...
		case 2 :
			datenow = Util.add0(temptoday1.get(Calendar.YEAR), 4) + "-"
					+ Util.add0(temptoday1.get(Calendar.MONTH) + 1, 2);
			...
			dateto = Util.add0(temptoday2.get(Calendar.YEAR), 4) + "-"
					+ Util.add0(temptoday2.get(Calendar.MONTH) + 1, 2);
		...
		case 3 :
			datenow = Util.add0(temptoday1.get(Calendar.YEAR), 4) + "-"
					+ Util.add0(temptoday1.get(Calendar.MONTH) + 1, 2) + "-"
					+ Util.add0(temptoday1.get(Calendar.DATE), 2);
			...
			dateto = Util.add0(temptoday2.get(Calendar.YEAR), 4) + "-"
					+ Util.add0(temptoday2.get(Calendar.MONTH) + 1, 2) + "-"
					+ Util.add0(temptoday2.get(Calendar.DATE), 2);
	...
	String sql = "select a.id, a.name from MeetingRoom a where 1=1 " + MeetingShareUtil.getRoomShareSql(user)
			+ sqlwhere + " order by dsporder,name ";
...
<table class=M11 width="100%" border=0 cellspacing=0 cellpadding=0
	Style="width: 100%; font-size: 8pt; table-layout: fixed">
	<COLGROUP>
		<col width="221">
			<col width="">
				<tr class="thbgc">
					<td class="schtd" align=center
						style="width: 220px; background: #f7f7f7; BORDER-left: 1px solid #d0d0d0; height: 25px;">
						<table height="100%" width="100%" border=0 cellspacing=0
							cellpadding=0>
							<tr>
								<td width="20px" style="BORDER-bottom: 0px; BORDER-LEFT: 0px">
									<button class="Browser" id=namebtn name=namebtn type="button"
										style="margin-top: 3px !important;" onclick=""></button>
								</td>
								<td width="200px" style="BORDER-bottom: 0px; BORDER-LEFT: 0px">
									<INPUT id=content name=content class="searchInput"
									onmouseover="this.select()" onfocus="clearVal(this);"
									onblur="recoverVal(this);" value="<%=content%>"
									style="width: 180px; height: 23px; border: 0px; background: #f7f7f7; color: #606060" />
								</td>
							</tr>
						</table>
					</td>
					...
					<%
						if (i == 0) {
					%>
					<td class="thcls" style="BORDER-left: 1px solid #d0d0d0;"
						align=center><%=i + 1%></td>
					<%
						} else {
					%>
...
<div id="tabledivData"
	style="overflow-y: hidden; overflow-x: hidden; position: relative; width: 100%">
	<div id="tableChlddivData">
		<table class=M1 width="100%" border=0 cellspacing=0 cellpadding=0
			Style="width: 100%; font-size: 8pt; table-layout: fixed;">
			<COLGROUP>
				<col width="221">
					<col width="">
						...
						%>
						<tr>
							<td class="roomnames"><div class="tdfcs"></div>
								<div class="tdtxt"
									title="<%=mrr.getMeetRoomInfo("" + tmproomid, MeetingRoomComInfo, user)%>"
									onclick="showRoomMeetingList('','',<%=tmproomid%>,event)">
									&nbsp;<%=Util.forHtml(Util.toScreen(MeetingRoomComInfo.getMeetingRoomInfoname("" + tmproomid),
							user.getLanguage()))%></div></td>
							<%
										...
										for (int j = 0; j < daysOfThisMonth; j++) {
											...
											for (int h = 0; h < ids.size(); h++) {
												...
												if (tmpdate.compareTo(beginDate) >= 0 && tmpdate.compareTo(endDate) <= 0) {
													...
													cnt++;
													if (tdTitle.equals("")) {
														tdTitle = mrr.getMeetRoomUseCase(name, totalmember, caller, contacter, beginDate,
																endDate, begintime, endtime, user);
													} else {
														tdTitle += "\n" + "----------------------------------------------------------"
																+ "\n" + mrr.getMeetRoomUseCase(name, totalmember, caller, contacter,
																		beginDate, endDate, begintime, endtime, user);
													}
												}
											}
							...
							%>
							<td class="tdcls" bgcolor="<%=bgcolor%>" align=center
				...
				<col width="221">
					<col width="">
						<tr class="thbgc">
							...
							%>
							<td class="thcls" style="BORDER-left: 1px solid #d0d0d0;"
								align=center><span class="weekclass"
								val="<%=TimeUtil.dateAdd(cBeginDate, i)%>"></span></td>
							<%
								} else {
							%>
							<td class="thcls" style="BORDER-LEFT: 0px" align=center><span
								class="weekclass" val="<%=TimeUtil.dateAdd(cBeginDate, i)%>"></span>
							</td>
							<%
								}
							%>
					...
					<COLGROUP>
						<col width="221">
							...
							user.getLanguage()))%></div></td>
									...
									%>
									<td class="tdcls" align=center bgcolor="<%=bgcolor%>"
										<%if (!"".equals(tdTitle)) {%> title="<%=tdTitle%>" <%}%>>
										<%
											if (cnt > 1) {
										%><%=cnt%>
						...
						<col width="221">
							<col width="">
								<tr class="thbgc">
										...
										<table height="100%" width="100%" border=0 cellspacing=0
											...
											<tr>
												<td width="20px"
													...
													<button class="Browser" id=namebtn name=namebtn
														type="button" style="margin-top: 3px !important;"
														onclick=""></button>
												</td>
												<td width="200px"
													style="BORDER-bottom: 0px; BORDER-LEFT: 0px"><INPUT
													id=content name=content class="searchInput"
													onmouseover="this.select()" onfocus="clearVal(this);"
													onblur="recoverVal(this);" value="<%=content%>"
													style="width: 180px; height: 23px; border: 0px; background: #f7f7f7; color: #606060" />
												</td>
									...
									%>
									<td class="thcls" style="BORDER-left: 1px solid #d0d0d0;"
										colspan="<%=dspUnit%>" align=center><%=i%></td>
									<%
										} else {
									%>
									<td class="thcls" style="BORDER-LEFT: 0px"
										colspan="<%=dspUnit%>" align=center><%=i%></td>
									<%
										}
									%>
							...
							user.getLanguage()))%></div></td>
											<%
												HashMap tempMap = (HashMap) mrrHash.get((String) meetingroomids.get(k));
														...
														if (ids.size() == 0) {
															for (int p = meetingSetInfo.getTimeRangeStart()
																	* dspUnit; p < (meetingSetInfo.getTimeRangeEnd() + 1) * dspUnit; p++) {
																out.println("<td class='tdcls' roomid='" + tmproomid + "' id='" + tmproomid
																		+ "' style=\"color:#fff;"
																		+ (dspUnit != 1 && p % dspUnit != 0 ? "border-left: 0px;" : "")
																		+ "\"  onselectstart='return false' target='" + p
																		+ "' onmousedown='createMeetingAction.startDrag(event);' >&nbsp;</td>");
															}
														...
														ArrayList cancels = (ArrayList) tempMap.get("cancels");
														ArrayList meetingStatuss = (ArrayList) tempMap.get("meetingStatus");
														for (int j = meetingSetInfo.getTimeRangeStart()
																* dspUnit; j < (meetingSetInfo.getTimeRangeEnd() + 1) * dspUnit; j++) {
															String bgcolor = "";
															String tdTitle = "";

															String tempTimeBg = datenow + " "
																	+ (dspUnit == 1 ? (Util.add0(j, 2) + ":00") : getTimesBg(j - 1, dspUnit));
															String tempTimeed = datenow + " "
																	+ (dspUnit == 1 ? (Util.add0(j, 2) + ":59") : getTimesEd(j, dspUnit));

															String temp = getHourOccupied(datenow, "" + j, beginDates, begintimes, endDates, endtimes,
																	cancels, dspUnit);
															if ("2".equals(temp)) {
																tdTitle = SystemEnv.getHtmlLabelName(82890, user.getLanguage());
															}
															...
															for (int h = 0; h < ids.size(); h++) {
																...
																if ((tempTimeed).compareTo(tempBeginDateTime) >= 0
																		&& (tempTimeBg).compareTo(tempEndDateTime) < 0) {
																	if ("1".equals(meetingStatus)) {
																		existDSP = true;
																	}
																	cnt++;
											...
											%>
											<td class="tdcls" roomid="<%=tmproomid%>"
												style="<%=(dspUnit != 1 && j % dspUnit != 0 ? "border-left: 0px;" : "")%>"
												align=center bgcolor="<%=bgcolor%>" id="<%=tmproomid%>"
												target="<%=j%>" onselectstart="return false"
												onmousedown='createMeetingAction.startDrag(event);'
												<%if (!"".equals(tdTitle)) {%> title="<%=tdTitle%>" <%}%>>
												<%
													if (cnt > 1) {
												%><%=cnt%>
...
function afterLoadDate(){
	...
	<%String subidsSpan = SystemEnv.getHtmlLabelName(141, user.getLanguage());
			if (subids > 0) {
				...
				if (RecordSet.next()) {
					subidsSpan = SystemEnv.getHtmlLabelName(140, user.getLanguage()) + " : "
							+ Util.null2String(RecordSet.getString("companyname"));
				}
			}%>
